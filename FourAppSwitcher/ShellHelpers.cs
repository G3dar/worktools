...
public static class ShellHelpers
{
    ...
    public static void BringToFront(Process process)
    {
        try
        {
            ...
            if (hWnd == IntPtr.Zero)
            {
                // try get from visible windows of this process
        ...
        try
        {
            EnumWindows((hWnd, l) =>
            {
                ...
                if (pid == (uint)processId)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(title)) result.Add((hWnd, title));
                }
                return true;
            }, IntPtr.Zero);
        }
    ...
    private static IEnumerable<IntPtr> EnumerateWindowsForProcess(int processId)
    {
        ...
        {
            foreach (var hWnd in EnumerateThreadWindows(thread.Id))
            {
                if (IsWindowVisible(hWnd)) yield return hWnd;
            }
        }
    }
    ...
    private static IEnumerable<IntPtr> EnumerateThreadWindows(int threadId)
    {
        var list = new List<IntPtr>();
        EnumThreadWindows(threadId, (hWnd, l) => { list.Add(hWnd); return true; }, IntPtr.Zero);
        return list;
    }
    ...
    public static (string? target, string? args, string? workDir) ResolveShortcutIfNeeded(string inputPath)
    {
        ...
        if (ext != ".lnk")
        {
            return (inputPath, null, Path.GetDirectoryName(inputPath));
        }
        try
        {
